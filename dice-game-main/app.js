...
(() => {
  ...
  function resetAll(){
    ...
    setTimeout(()=> { cube.style.transition = 'transform 1.25s cubic-bezier(.18,.9,.32,1)'; }, 700);
  }
  ...
  diceArea.addEventListener('click', () => {
    ...
    const finalX = currentRotation.x + normalizeDelta(base.x - mod(currentRotation.x,360)) + extraX;
    const finalY = currentRotation.y + normalizeDelta(base.y - mod(currentRotation.y,360)) + extraY;
    ...
    const onEnd = () => {
      ...
      statusEl.innerHTML = `You Enter ${your} and Dice Roll ${ your === opp ? `<strong>${opp}</strong>` : `<strong style="color:${opp>0?'#c0392b':''}">${opp}</strong>` }`;
      // clear selection
      selectedNumber = null;
      document.querySelectorAll('.num').forEach(n => n.classList.remove('active'));
      rolling = false;
